...
function update {
    ...
    local Usage="Use update 'Remote Name' 'Repository Url' 'Target File/Folder' ( 'Source File/Folder' )"
    ...
    if [ -z "$target" ] ; then
        alert "The <Target File/Folder> parameter is missing"
        print "$Usage"
        return 1
    fi
